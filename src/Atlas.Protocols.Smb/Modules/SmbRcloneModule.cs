...
/// <summary>
/// </summary>
public sealed class SmbRcloneModule : AtlasModule<Smb2Client>
{
    public override string Name => "rclone";
    public override string Description => "Searches for rclone.conf";
    ...
    public override async Task RunAsync(AtlasModuleContext<Smb2Client> ctx, CancellationToken cancellationToken)
    {
        ...
        string[] patterns = new[] { "rclone.conf" };
        int found = 0;
        ...
        AtlasConsole.Info($"{ctx.Host}:445", $"(rclone) {found} file(s)");
    }
        ...
        while (queue.Count > 0 && visited < maxFiles)
        {
            ...
            foreach (var e in entries)
            {
                ...
                if (!e.IsDirectory)
                {
                    ...
                    {
                        AtlasConsole.Success($"{ctx.Host}:445", $"(rclone) {share}\\{child} ({e.Size} bytes)");
                        count++;
                    }
                }
