...
/// <summary>
/// </summary>
public sealed class SmbNotepadPlusPlusModule : AtlasModule<Smb2Client>
{
    public override string Name => "notepad++";
    public override string Description => "Extracts notepad++ unsaved";
    ...
    public override async Task RunAsync(AtlasModuleContext<Smb2Client> ctx, CancellationToken cancellationToken)
    {
        ...
        AtlasConsole.Info($"{ctx.Host}:445", $"(notepad++) {found} file(s)");
    }
        ...
        while (queue.Count > 0 && visited < maxFiles)
        {
            ...
            foreach (var e in entries)
            {
                ...
                if (!e.IsDirectory)
                {
                    ...
                    {
                        AtlasConsole.Success($"{ctx.Host}:445", $"(notepad++) {share}\\{child} ({e.Size} bytes)");
                        count++;
                    }
                }
