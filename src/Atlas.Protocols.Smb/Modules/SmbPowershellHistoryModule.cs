...
/// <summary>
/// </summary>
public sealed class SmbPowershellHistoryModule : AtlasModule<Smb2Client>
{
    public override string Name => "powershell_history";
    public override string Description => "Extracts PowerShell history";
    ...
    public override async Task RunAsync(AtlasModuleContext<Smb2Client> ctx, CancellationToken cancellationToken)
    {
        ...
        string[] patterns = new[] { "ConsoleHost_history.txt" };
        int found = 0;
        ...
        AtlasConsole.Info($"{ctx.Host}:445", $"(powershell_history) {found} file(s)");
    }
        ...
        while (queue.Count > 0 && visited < maxFiles)
        {
            ...
            foreach (var e in entries)
            {
                ...
                if (!e.IsDirectory)
                {
                    ...
                    {
                        AtlasConsole.Success($"{ctx.Host}:445", $"(powershell_history) {share}\\{child} ({e.Size} bytes)");
                        count++;
                    }
                }
