...
/// <summary>
/// </summary>
public sealed class SmbVncModule : AtlasModule<Smb2Client>
{
    public override string Name => "vnc";
    public override string Description => "Loot VNC passwords";
    ...
    public override async Task RunAsync(AtlasModuleContext<Smb2Client> ctx, CancellationToken cancellationToken)
    {
        ...
        string[] patterns = new[] { "vnc.ini", ".vnc" };
        int found = 0;
        ...
        AtlasConsole.Info($"{ctx.Host}:445", $"(vnc) {found} file(s)");
    }
        ...
        while (queue.Count > 0 && visited < maxFiles)
        {
            ...
            foreach (var e in entries)
            {
                ...
                if (!e.IsDirectory)
                {
                    ...
                    {
                        AtlasConsole.Success($"{ctx.Host}:445", $"(vnc) {share}\\{child} ({e.Size} bytes)");
                        count++;
                    }
                }
