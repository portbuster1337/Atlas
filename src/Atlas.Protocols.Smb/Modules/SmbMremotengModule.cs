...
/// <summary>
/// </summary>
public sealed class SmbMremotengModule : AtlasModule<Smb2Client>
{
    public override string Name => "mremoteng";
    public override string Description => "Dump mRemoteNG";
    ...
    public override async Task RunAsync(AtlasModuleContext<Smb2Client> ctx, CancellationToken cancellationToken)
    {
        ...
        AtlasConsole.Info($"{ctx.Host}:445", $"(mremoteng) {found} file(s)");
    }
        ...
        while (queue.Count > 0 && visited < maxFiles)
        {
            ...
            foreach (var e in entries)
            {
                ...
                if (!e.IsDirectory)
                {
                    ...
                    {
                        AtlasConsole.Success($"{ctx.Host}:445", $"(mremoteng) {share}\\{child} ({e.Size} bytes)");
                        count++;
                    }
                }
