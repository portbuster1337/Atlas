...
/// <summary>
/// </summary>
public sealed class SmbIisModule : AtlasModule<Smb2Client>
{
    public override string Name => "iis";
    public override string Description => "Checks for IIS Application Pool";
    ...
    public override async Task RunAsync(AtlasModuleContext<Smb2Client> ctx, CancellationToken cancellationToken)
    {
        ...
        string[] patterns = new[] { "applicationHost.config" };
        int found = 0;
        ...
        AtlasConsole.Info($"{ctx.Host}:445", $"(iis) {found} file(s)");
    }
        ...
        while (queue.Count > 0 && visited < maxFiles)
        {
            ...
            foreach (var e in entries)
            {
                ...
                if (!e.IsDirectory)
                {
                    ...
                    {
                        AtlasConsole.Success($"{ctx.Host}:445", $"(iis) {share}\\{child} ({e.Size} bytes)");
                        count++;
                    }
                }
